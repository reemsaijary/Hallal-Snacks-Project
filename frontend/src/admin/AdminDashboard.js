...
import { Box, Container, Grid, Paper, Typography, Card, CardContent, IconButton, Button, Tab, Tabs, TextField, Dialog, DialogTitle, DialogContent, DialogActions, Divider } from '@mui/material';
      ...
      <Container maxWidth="xl">
        ...
        {tab === 0 && (
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={4}>
              <Card><CardContent>
                <Typography variant="h6">Top 3 Most Required Items</Typography>
          ...
          <Box>
            ...
            {categories.map(cat => (
              <Paper key={cat} sx={{ p: 2, mb: 4 }}>
                <Typography variant="h6" sx={{ mb: 2, color: '#d32f2f' }}>{cat} Section</Typography>
                <Box sx={{ height: 300 }}><DataGrid rows={products.filter(p => p.category === cat)} columns={productCols} /></Box>
              </Paper>
            ))}
          </Box>
        )}
        ...
        <Dialog open={open} onClose={() => setOpen(false)}>
          ...
          <DialogContent sx={{ pt: 2 }}>
            <TextField fullWidth label="Name" value={selectedProduct.name} onChange={(e) => setSelectedProduct({...selectedProduct, name: e.target.value})} sx={{ mb: 2 }} />
            ...
            <TextField fullWidth label="Category" value={selectedProduct.category} helperText="ChickenBurger, BeefBurger, Sandwiches, or Fries" onChange={(e) => setSelectedProduct({...selectedProduct, category: e.target.value})} sx={{ mb: 2 }} />
            <TextField fullWidth label="Ingredients" multiline value={selectedProduct.ingredients} onChange={(e) => setSelectedProduct({...selectedProduct, ingredients: e.target.value})} />
          </DialogContent>
          <DialogActions><Button onClick={() => setOpen(false)}>Cancel</Button><Button onClick={handleSave} variant="contained">Save</Button></DialogActions>
        </Dialog>
      </Container>
    </Box>
  );
